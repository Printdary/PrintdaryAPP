...
{
    return [self initWithQuality:AVCaptureSessionPreset1280x720 position:LLCameraPositionRear videoEnabled:videoEnabled];
}
...
- (void)handlePinchGesture:(UIPinchGestureRecognizer *)recognizer
{
    ...
    if (allTouchesAreOnThePreviewLayer) {
        ...
        if (_effectiveScale < 1.0f)
            _effectiveScale = 1.0f;
        if (_effectiveScale > self.captureDevice.activeFormat.videoMaxZoomFactor)
            _effectiveScale = self.captureDevice.activeFormat.videoMaxZoomFactor;
        NSError *error = nil;
        if ([self.captureDevice lockForConfiguration:&error]) {
            [self.captureDevice rampToVideoZoomFactor:_effectiveScale withRate:100];
...
- (void)initialize
{
    if(!_session) {
        ...
        NSError *error = nil;
        _videoDeviceInput = [AVCaptureDeviceInput deviceInputWithDevice:_captureDevice error:&error];
...
-(void)capture:(void (^)(LLSimpleCamera *camera, UIImage *image, NSDictionary *metadata, NSError *error))onCapture exactSeenImage:(BOOL)exactSeenImage animationBlock:(void (^)(AVCaptureVideoPreviewLayer *))animationBlock
{
    ...
    BOOL flashActive = self.captureDevice.flashActive;
...
- (void)setVideoCaptureDevice:(AVCaptureDevice *)captureDevice
{
    ...
    if(self.onDeviceChange) {
        __weak typeof(self) weakSelf = self;
        self.onDeviceChange(weakSelf, captureDevice);
    }
}

- (BOOL)isFlashAvailable
{
    return self.captureDevice.hasFlash && self.captureDevice.isFlashAvailable;
}

- (BOOL)isTorchAvailable
{
    return self.captureDevice.hasTorch && self.captureDevice.isTorchAvailable;
}

- (BOOL)flashMode:(LLCameraFlash)cameraFlash
{
    if(!self.session)
        return NO;
...
- (UIImage *) imageFromSampleBuffer:(CMSampleBufferRef) sampleBuffer
{
    ...
    CGImageRef newImage = [self.context
                          createCGImage:ciImage
                          fromRect:CGRectMake(0, 0,
                                              CVPixelBufferGetWidth(imageBuffer),
                                              CVPixelBufferGetHeight(imageBuffer))];
    UIImage * img = [UIImage imageWithCGImage:newImage scale:1.0 orientation:UIImageOrientationUp];
...
+ (UIImage *) imageFromSampleBuffer:(CMSampleBufferRef) sampleBuffer cont: (CIContext*)context
{
    CVImageBufferRef imageBuffer = CMSampleBufferGetImageBuffer(sampleBuffer);
    CIImage *ciImage = [[CIImage alloc]initWithCVImageBuffer:imageBuffer];
    
    
    CGImageRef newImage = [context
                           createCGImage:ciImage
                           fromRect:CGRectMake(0, 0,
                                               CVPixelBufferGetWidth(imageBuffer),
                                               CVPixelBufferGetHeight(imageBuffer))];
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection{
    ...
    [self.imageCaptureDelegate capturedImage:img];
