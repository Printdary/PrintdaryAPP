...
- (void)initialize
{
	self.font = self.font ?: [UIFont fontWithName:@"HelveticaNeue-Light" size:8];
	self.highlightedFont = self.highlightedFont ?: [UIFont fontWithName:@"HelveticaNeue" size:10];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section
{
    ...
    NSLog(@"hhhh %f",[self.dataSource numberOfItemsInPickerView:self]);
...
- (void)initialize
{
	...
	self.label = [[UILabel alloc] initWithFrame:CGRectMake(0, self.contentView.frame.size.height - 20, self.contentView.frame.size.width, self.contentView.frame.size.height - 40)];
	...
	self.label.font = [UIFont systemFontOfSize:6];
...
- (void)setSelected:(BOOL)selected
{
    ...
    self.imageView.alpha = self.selected ? 1 : 0.5;
