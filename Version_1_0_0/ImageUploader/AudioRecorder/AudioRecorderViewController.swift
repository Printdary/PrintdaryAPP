...
class AudioRecorderViewController: UIViewController,UITextFieldDelegate, AVAudioRecorderDelegate,EZMicrophoneDelegate {
  ...
  func drawBufferPlot(){
    ...
    func uploadRecordingToFirebase(){
        let storageRef = Storage.storage().reference().child("audioRecording/\(Date()).m4a")
        // Upload the file to the path "images/rivers.jpg"
        let uploadTask = storageRef.putFile(from: self.audioFileName!, metadata: nil) { metadata, error in
            if let error = error {
                ...
                self.saveDawnloadUrlToFirebase(downloadURL: downloadURL! as NSURL)
            }
        }
    }
    
    
    func saveDawnloadUrlToFirebase(downloadURL: NSURL){
      
        let dbRef = Database.database().reference().child("Audio").child(UserModel.currentUser.uploadID).childByAutoId()
        ...
        audioObj.key = dbRef.key
        audioObj.audioURL = downloadURL.absoluteString
