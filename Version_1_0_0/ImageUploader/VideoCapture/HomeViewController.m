...
@interface HomeViewController ()<LLSimpleCameraDelegate>
...
-(void)capturedImage:(UIImage *)image {
    ...
    UIImage * resizedImage = [self imageWithImage:image convertToSize:CGSizeMake(85, 50)];
    _imageCounter++;
    if (_imageCounter > 10) {

        self.imageCapturedBlock(resizedImage);
    }
}

-(void) capturedImageData:(NSData *)imageData {
    self.capturedBlock(imageData);
}
