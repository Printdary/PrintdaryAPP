...
class MSCropView: UIView, UIGestureRecognizerDelegate {
    ...
    private func addCropView() {
        ...
        cropView.frame = CGRect(x: (imageView.image?.size.width)!/2, y: (imageView.image?.size.height)!/2, width: 300, height: 300)
    ...
    private func adjustAnchorPoint(for gestureRecognizer: UIGestureRecognizer) {
     ...
     func cropImage() {
        ...
        self.croppedIm = croppedImage;
       
        NotificationCenter.default.post(name: Notification.Name.init("CROP_PRESSED"),
                                        object: nil,
                                        userInfo: ["cropedImage" : croppedImage as? UIImage])
